...
class WaitlistEventsHandler: NSObject, FlutterStreamHandler, RtkEventsHandler {
    var eventSinkWrapper: EventSinkWrapper?
    var rtkClient: RtkClient
    var listener: WaitingRoomEventListener?
    ...
    func initializeHandler() {
        if let sink = defaultSink {
            ...
            listener = WaitingRoomEventListener(sink: eventSinkWrapper!)
            sendNotification()
        }
    }
    ...
    func disposeHandler() {
        if let _listener = listener {
            rtkClient.removeWaitlistEventListener(waitlistEventListener: _listener)
    ...
    func sendNotification() {
        NotificationCenter.default.post(Notification(name: Notification.Name(waitlistNotification), object: listener))
    }
