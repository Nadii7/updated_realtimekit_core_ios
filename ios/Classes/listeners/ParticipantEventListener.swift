...
class ParticipantEventsHandler: NSObject, FlutterStreamHandler, RtkEventsHandler {
    var eventSinkWrapper: EventSinkWrapper?
    var rtkClient: RtkClient
    var listener: ParticipantEventListener?
    ...
    func initializeHandler() {
        if let sink = defaultSink {
            ...
            listener = ParticipantEventListener(sink: eventSinkWrapper!)
            sendNotification()
        }
    }
    ...
    func disposeHandler() {
        if let _listener = listener {
            rtkClient.removeParticipantsEventListener(participantEventListener: _listener)
    ...
    func sendNotification() {
        NotificationCenter.default.post(Notification(name: Notification.Name(participantNotification), object: listener))
    }
